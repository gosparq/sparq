# -----------------------------------------------------------------------------
# sparQ — Tasks: My Inbox projection queries (DB Access Standards §5.2)
...
class MyTaskRow:
    """Flat projection of a task assigned to the current member."""
...
def get_mine_open(member_id: int, organization_id: UUID, workspace_id: UUID) -> list[MyTaskRow]:
    """Return open action items assigned to the given member."""
    ...
    return [MyTaskRow(*row) for row in db.session.execute(stmt).all()]


def get_mine_closed(member_id: int, organization_id: UUID, workspace_id: UUID) -> list[MyTaskRow]:
    """Return closed action items assigned to the given member."""
    from modules.base.tasks.models.task import Task
    from modules.base.core.models.workspace_user import WorkspaceUser
    from modules.base.core.models.user import User
    from modules.base.projects.models.project import Project
